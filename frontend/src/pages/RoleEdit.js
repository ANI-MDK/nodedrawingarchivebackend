...
const RoleEdit = () => {
    ...
    //console.log(state);
    let roleName= (state!==null? state.row.role_name : "")
    ...
    const crumbs = [
        ...
        { label: 'Edit Role', path: '/roleedit' }
    ]

    useEffect(()=>{
        (permissions!==null && permissions.is_admin_user==="Y") ? navigate("/roleedit",{state}) : navigate("/dashboard")
    },[]);
    ...
    const handelRoleEditSubmit = (e) => {
        //console.log(role);
        e.preventDefault(); 
        axios.put(`${baseUrl}/role//updaterole/${state.row.role_id}`,{role_name:role},
            {headers: { 
                        ...
                        <div className="container mt-8">
                            <div className="flex flex-col items-center">
                                <div className="w-full max-w-5xl bg-cyan-700 bg-opacity-70 shadow-lg rounded-lg p-5 my-2">
                                    <h3 className="block text-lg font-medium text-gray-700 mb-1">Role Edit Section:</h3>
                                    <form onSubmit={handelRoleEditSubmit}>
                                        <div className="bg-gray-100 shadow-lg rounded-lg p-5 mb-2">
                                            <div className="flex flex-wrap -mx-3 mb-2">
                                                <div className="w-full px-3 mb-2">
                                                    ...
                                                    <input type="text" id="name" name="name" value={role} placeholder="Enter Role Name here" onChange={handleRoleEditChange} className="block mt-1 p-2 w-full border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-500 focus:border-gray-500" required/>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="mt-4">
